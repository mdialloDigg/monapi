...
<style>
body{text-align:center;font-family:Arial;background:#eef2f7;padding-top:60px}
input,button{padding:12px;font-size:16px;width:90%;max-width:320px}
button{background:#007bff;color:white;border:none}
</style>
</head>
<body>
<h3>📞 Numéro de téléphone expéditeur</h3>
<form method="post" action="/users/lookup">
<input name="phone" placeholder="Ex: 622000000" required><br><br>
...
app.post('/users/lookup', async (req, res) => {
  const user = await User.findOne({ senderPhone: req.body.phone }).sort({ createdAt: -1 });
  req.session.prefill = user || null;
...
<h4>📤 Expéditeur</h4>
<input id="senderFirstName" value="${u.senderFirstName||''}" placeholder="Prénom" required>
<input id="senderLastName" value="${u.senderLastName||''}" placeholder="Nom" required>
<input id="senderPhone" value="${u.senderPhone||''}" placeholder="Téléphone" required>

<select id="originLocation" required>
<option>${u.originLocation||'Origine'}</option>
...
<div class="box dest">
<h4>📥 Destinataire</h4>
<input id="receiverFirstName" value="${u.receiverFirstName||''}" placeholder="Prénom" required>
<input id="receiverLastName" value="${u.receiverLastName||''}" placeholder="Nom" required>
<input id="receiverPhone" value="${u.receiverPhone||''}" placeholder="Téléphone" required>

<select id="destinationLocation" required>
<option>${u.destinationLocation||'Destination'}</option>
<option>France</option><option>Labé</option><option>Belgique</option>
<option>Conakry</option><option>Suisse</option><option>Atlanta</option>
<option>New York</option><option>Allemagne</option>
</select>
...
<option>${u.recoveryMode||'Mode de retrait'}</option>
...
const w=window.open('');
w.document.write('<h3>🧾 Reçu</h3><p><b>Code:</b> '+lastCode+'</p><p><b>Destination:</b> '+destinationLocation.value+'</p>');
w.print();
}
</script>
